...
Hmm, we get two values as `range()` actually returns the minimum and maximum values in the `salary` column. We get the properly defined range by subtraction
...
We've broken a key rule, however! We haven't visualised the distribution... Let's plot the data. Given we have two groups (two `Position` groups), we should plot these individually and together to see how they differ. 
...
Ah, so the two local maxima seem to come broadly from the two classes of position when you plot them individually. Let's get separate ranges for these salaries grouped by position
...
<br>
<div class='emphasis'>
We already see a big problem with using the range as a measure of the spread in a distribution: it's **very strongly affected by outliers!**
</div>
<br>
...
* *General quantile*, e.g. the $43%$ quantile = the value splitting the distribution into a lower $43\%$ and higher $57\%$
...
The IQR is the range in which $50\%$ of the values fall. It is a useful measure of spread, as it's **insensitive to outliers**. R also gives us an `IQR()` function to use directly if you wanted to:
...
Precise definitions vary, but, for a data set $x$ the whiskers in `R` are positioned at
<br><br>
...
and this is a very common definition. Data points **below the lower whisker** or **above the upper whisker** are deemed to be **outliers**.

<br>
If you were to look at how much data is contained in each section, it gives you a good idea of why a boxplot is a useful way to look at the distribution of your data. The key is to think carefully about **how much of the data is present in each section of the plot**.
...
Repeat the '25% partitioning' of the Task above for the boxplot of the skewed distribution. Think carefully about *where* the weight of the distribution is concentrated and try to make the link between what you see in the histogram and the features of the boxplot.
<details>
...
<details>
<summary> Maths notation </summary>
...
Using the same set of numbers $x = \{3, 1, 9, 4, 12\}$ evalute the sum 
$$\sum_{i=2}^{4}{x_{i}}$$
<details>
<summary>**Solution**</summary>
...
# Standard deviation - an interpretable single number measure of spread
...
The standard devaiation is a quantity expressing by how much, on average, the values in a distribution differ from the distribution mean. In other words, how spread out are the observations?
