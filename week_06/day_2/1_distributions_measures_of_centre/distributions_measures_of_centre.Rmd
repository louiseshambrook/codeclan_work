...
So far this week we've learnt about probability as a concept, and how to calculate the probability of an observation occuring within a dataset. In practice though we often come across the concept of a  probability **distribution**. In the following lessons, we will learn more about what a probability distribution is, what kind of distributions we encounter, and why knowing about them is incredibly useful for data analysis. 

# What is a distribution? 

Yesterday we learned about random variables and their associated frequency and probability. For example, a random variable could be the outcome of the roll of a die, the flip of a coin, or the colour blindness status of an individual. 
...
Now we have our distribution, we can start using it to summarise the data. To start with, we're going to look at three measures of the 'centre' of a probability distribution. Each of the measures have their own advantages and disadvantages, which we will talk about later. First, we'll define the measures.
...
Next, what we need is the total number of occurrences in our data. For this, we can use the `tabyl()` function we saw yesterday in the lesson on conditional probability. This time we'll use it on a single variable `units_sold`, which is discrete numerical. 
...
```
<br> 
In the first row we have the number of units sold, and on the second row, the number of dates on which that number of sales occurred. The entries on the second row we can think of as **frequencies**, e.g. there were 8 days on which 4 units were sold in this dataset.

The last column is a bit misleadingly titled *'percent'*: it is the relative frequency or proportion, i.e. the count `n` divided by the `sum(n)` 
...
The best way to understand this is via an example. Imagine we have an unbiased die, which we roll $1000$ times, noting the results as we go. We know that the **theoretical probability** of each number is $1/6 = 0.166667$. However, the frequencies we actually rolled are:
...
freqs
```
We get the **relative frequencies** by dividing the observed frequencies by the total number of rolls, which is what we see in the `percent` column, We can interpret these as **empirical probabilities**, 'empirical' in the sense of 'obtained from observation'. 

**If all we knew about this die were the results of these $1000$ rolls, the empirical probabilities would be our best estimate of the behaviour of the die**.
...
Now we have our distribution, we can start looking at summaries of that distribution. First, definitions. 
<br> 



**Mean**: The mean is just another term for the average, defined as
...
The **median** is the value for which one-half of the values in the data set lie below it, and one-half above. R makes this easy with the `median()` function! *(Note: We had an odd number of values in `AirConSales` CSV, so it was easy enough to find the half way. If you have an odd number, you average the two middle values)* 

<br>

**Mode**

The **mode** is the most likely value in the data set, i.e. the value that occurs most frequently. R has no built-in function to calculate the mode of a distribution (this is genuinely weird), but we can create one ourselves. We'll use this in a minute to calculate all our stats. 
...
Let's calculate all these values now:
...
```
So these are our results, and you can see that they are pretty different... if we look at the mean, we have a  higher value than the median and mode. Why is this?
...
Outliers are values that are a bit further away from the bulk of the data. Why mention outliers here? It turns out that **the three measures of distribution 'centre' show very different sensitivities to outliers!**

</div>
<br>

The key point is that **the mean is more heavily swayed by outliers than the median or the mode**. If we suspect our data set contains outliers, we should consider carefully which measure of centrality to use, and perhaps prefer the median to the mean. You can check whether or not your data contains outliers by plotting it and looking at the **distribution shape.**



<hr>

# Distribution shapes: unimodal and bimodal
...
knitr::include_graphics("images/unimodal.png")
```
...
```{r, echo = FALSE, out.width = '80%'}
knitr::include_graphics("images/bimodal.png")
```
...
Look back at the `AirConSales` distribution of daily sales figures and plot a histogram of the data. Is this unimodal or bimodal? 
...
It's difficult to say clearly, but it seems to look like a unimodal distribution with an outlier (the day there was lots of aircon unit sales). Could be an argument that maybe it is bimodal... 
...
The shape of the distribution helps decide which measure of centrality to use. But there is more...

<hr>

# Skewness

Not only can we describe the central tendencies of a distribution, as well as how many peaks it has, but we can describe how symmetrical it is. This is called **skewness**.   
...
Left skewed data will show the bulk of the data on the right of the plot, and the tail extending out to the left. 

```{r, echo = FALSE, out.width = '80%'}
knitr::include_graphics("images/left_skew.png")
```
...
```{r, echo = FALSE, out.width = '80%'}
knitr::include_graphics("images/right_skew.png")
```

The measures fall in the order $\textrm{mode} \lt \textrm{median} \lt \textrm{mean}$. 




<details><summary>**Optional - binning a continuous variable**</summary>
...
left_skewed %>%
  ...
  summarise(mode_daily_sales = get_mode(binned))
```
...
```{r, message=FALSE}
library(e1071) # rolls of the tongue, doesn't it 
```
